class DatabaseHelperStub:
    ...
    product_orders = [[('Beach Towel', 5, 0, 15.99), ('Sunglasses', 2, 5, 25.5), ('Volley Ball', 1, 5, 19.99),
                       ('Surf Board (Adult)', 1, 0, 121.99), ('Inflatable Pool Float', 1, 5, 18.99)],
                      [('Pedal Boat', 1, 0, 1899.99), ('Sunglasses', 2, 5, 25.5), ('Volley Ball', 1, 5, 19.99),
                       ('Inflatable Pool Float', 1, 5, 18.99)]]
    ...
    def get_customer_products(self, order_id):
        return self.product_orders[order_id-1]

    def add_customer_order(self, first_name, last_name, passphrase, collection_date, products):
        customer_order_id = self.customer_orders[-1][0]+1
        self.customer_orders.append((customer_order_id, first_name, last_name, passphrase, collection_date))
        self.product_orders.extend([(customer_order_id, *product) for product in products])